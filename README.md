...
| `list_jobs` | List all jobs with pagination and sorting only. Use `search_jobs` when you have filter criteria. |
...
Most tools are **read-only** (`readOnlyHint: true`). `create_candidate`, `create_hotlist`, `add_records_to_hotlist`, `create_task`, `create_meeting`, and `create_note` are mutating tools and should only be used when explicitly requested by the user.
...
- `create_candidate`, `create_hotlist`, `add_records_to_hotlist`, `create_task`, `create_meeting`, and `create_note` are write tools; this extension still does not expose delete operations
